...
export const MetadataExtraction: React.FC = () => {
  ...
  const fetchJobStatuses = async () => {
    ...
    for (const conn of activeConnections) {
      try {
        const response = await axios.get(`${API_BASE}/api/admin/metadata/jobs/${conn.id}/status`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        setJobs(prev => ({ ...prev, [conn.id]: response.data }));
      } catch (error) {
  ...
  const startExtraction = async (connectionId: string) => {
    try {
      const token = await getAuthToken();
      await axios.post(`${API_BASE}/api/admin/metadata/connections/${connectionId}/extract`, {
        fullExtraction: true,
      ...
      alert('Failed to start extraction. Check console for details.');
    }
  };
