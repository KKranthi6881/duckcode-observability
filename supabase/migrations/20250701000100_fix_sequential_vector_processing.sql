...
BEGIN
    ...
    -- Priority 3: Vector-only jobs (completed documentation, EXPLICITLY pending vectors)
    -- FIXED: Only process vectors when vector_status is explicitly 'pending', not NULL
    IF selected_job_id IS NULL THEN
        SELECT pj.id INTO selected_job_id
        FROM code_insights.processing_jobs pj
        JOIN code_insights.files f ON pj.file_id = f.id
        WHERE pj.status = 'completed' 
          AND pj.vector_status = 'pending'  -- FIXED: No COALESCE, only explicit 'pending'
    ...
    IF selected_job_id IS NOT NULL THEN
        ...
        SET 
            ...
            END,
            vector_status = CASE 
                WHEN pj_update.status = 'completed' AND pj_update.vector_status = 'pending' THEN 'processing'  -- FIXED: No COALESCE
...
-- Add comment
COMMENT ON FUNCTION code_insights.lease_processing_job IS 'Fixed lease function for sequential processing - only processes vectors when explicitly pending, not NULL'; 