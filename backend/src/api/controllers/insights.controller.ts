...
export const processDocumentationOnly = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    console.log(`[InsightsController] Starting DOCUMENTATION-ONLY processing for repository: ${repositoryFullName} for user ${userId}${selectedLanguage ? ` with language: ${selectedLanguage}` : ''}`);
    ...
    if (error) {
      console.error('Supabase DB Error during file upsert:', error);
      throw new Error(`Failed to save file data: ${error.message}`);
    }

    console.log(`[InsightsController] Successfully upserted ${data?.length || 0} files for processing.`);

    // --- DOCUMENTATION-ONLY JOBS (No vector processing) ---
    if (data && data.length > 0) {
      const jobRecords = data.map(file => ({
        file_id: file.id,
        status: 'pending',
        vector_status: null, // ← NO VECTOR PROCESSING
        lineage_status: null, // ← NO LINEAGE PROCESSING  
        retry_count: 0,
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString(),
      ...
      console.log(`Creating ${jobRecords.length} DOCUMENTATION-ONLY processing jobs.`);
      ...
      if (jobError) {
        console.error('Supabase DB Error during documentation-only job creation:', jobError);
        throw new Error(`Failed to create documentation jobs: ${jobError.message}`);
      }

      console.log(`Successfully created ${jobRecords.length} documentation-only processing jobs.`);
    }

    res.status(202).json({
      message: 'Documentation-only processing initiated. Files have been queued for documentation analysis.',
      totalFilesScanned: allFiles.length,
      filesQueued: data?.length || 0,
      phase: 'documentation'
    });
    ...
    console.error(`[InsightsController] Error in processDocumentationOnly:`, error);
    next(error);
  }
};
...
export const processVectorsOnly = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    console.log(`[InsightsController] Starting VECTORS-ONLY processing for repository: ${repositoryFullName}`);

    // Find files with completed documentation but no vector processing
    ...
    // Update jobs to enable vector processing
    ...
    if (updateError) {
      console.error('Error updating jobs for vector processing:', updateError);
    ...
    res.status(202).json({
      message: 'Vector processing initiated for completed documentation files.',
      filesQueued: completedDocs.length,
      phase: 'vectors'
    });

  } catch (error: any) {
    console.error(`[InsightsController] Error in processVectorsOnly:`, error);
    next(error);
  }
};
